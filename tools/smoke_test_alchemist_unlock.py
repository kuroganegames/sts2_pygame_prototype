...
with tempfile.TemporaryDirectory() as tmp:
    ...
    assert not unlocks.is_unlocked(profile, "character", "alchemist")
    run = create_run(registry, "wanderer", seed=1802, run_config={"seed": 1802, "profile_eligible": True})
    ...
    assert unlocks.is_unlocked(profile, "character", "alchemist")
    assert any(rule.get("target_id") == "alchemist" for rule in progression.new_content_unlocks)
print("OK")
